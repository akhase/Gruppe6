...
- weniger Prameter
- mehr epochen
- nochmal relevante Zusammenhänge filtern -> cor oder heat
```{r}
...
#konervtieren zu npArray
training_features = pd.DataFrame(r.training_features)
training_labels = pd.DataFrame(r.training_labels)
validation_features = pd.DataFrame(r.validation_features)
validation_labels = pd.DataFrame(r.validation_labels)

model = Sequential([
  InputLayer(input_shape=(len(training_features.keys()), )),
  BatchNormalization(),
...
# Definition der Kosten-(Loss-)Funktion und der Optimierungsfunktion mit Hyperparametern
model.compile(loss="mse", optimizer=Adam(learning_rate=0.001))

# Schaetzung des Modells
history = model.fit(training_features, training_labels, epochs=150,
