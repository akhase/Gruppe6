...
- Wettercodes zusammenfassen-> weniger Detaillierung
- Datum könnte man herunterbrechen auf Monate -> weniger Detaillierung
...
```
```{r}
#Konfidenzintervall berechnen für Umsatz je nach Feiertag
#wieso sind es nur 22:,)
print(sum(df_all$Feiertage))
...
ggplot(my_konf)+
  geom_bar(aes(x=Feiertage, y=mean), stat= "identity", fill="blue", alpha=0.1)+
  geom_errorbar(aes(x=Feiertage, ymin=mean-ic, ymax=mean+ic), width=0.5, colour = "orange", alpha=0.9, size=1.5)
